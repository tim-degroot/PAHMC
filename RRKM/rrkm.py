...
for struct in args:
    ...
    for i,freq in enumerate(freqs):
        print(i+1,'vib',freq,0.,1,sep=',',file=fh_densfile)
...
rate[energ > barrier] = sos[1][energ < np.max(energ)-barrier]/dos[0][energ > barrier]
rate /= h
...
print("RRKM rate, sf={:.3f}, S*={:.2f}, E0={:.2f} and Delta={:.2f}".format(sf,DS,barrier,delta),file=fh_rrkmfile)
...
if outfile_rev:
    ...
    rate[energ > barrier] = sos[1][energ < np.max(energ)-barrier]/dos[2][energ > barrier]
    rate /= h
    ...
    print("RRKM rate, sf={:.3f}, S*={:.2f}, E0={:.2f} and Delta={:.2f}".format(sf,DS,barrier,delta),file=fh_rrkmfile_rev)
