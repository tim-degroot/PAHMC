...
class Input_reader():
    ...
    def __init__(self, filename):
        ...
        for i, edge in enumerate(self.mol_edge):
            ...
            for j, atoms in enumerate(edge):
                ...
                if atoms not in ('H','D','HH', 'HD', 'DD'):
                    logger.error('Incorrect atoms defined, only no atom (0), hydrogen (H), deuterium (D), or aliphatic groups (HH, HD, DD) are currently supported. ')
                    sys.exit(2)
        ...
        for i, link in enumerate(self.mol_links):
            ...
            if len(link) > 2:
                logger.error('Individual links can only have up to two components currently. ')
                sys.exit(2)
            for j, link_ in enumerate(link):
                if link_ != '0':
                    logger.error('Incorrect link specified, only empty tertiary carbons (0) are accepted. ')
                    sys.exit(2)
                link[j] = link_
        ...
        if isinstance(data.get('Energy range'), float):
            ...
            if len(self.energy_range) == 1:
                ...
                logger.error('Cannot understand the simulation range given, it should be either a single energy or an energy range (min, max, nsteps). ')
        ...
        if not os.path.isdir(self.rate_dir):
            logger.error('The provided rates directory \''+self.rate_dir+'\'  was not found.')
            sys.exit(2)
