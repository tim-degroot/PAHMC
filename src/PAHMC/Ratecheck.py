...
def Determine_needed_rates(deuterium,edge_numbering, link_numbering):
    ...
    all_reactions = []
    length = len(edge_numbering) # both edges and links are same length (checked in input function)
    
    
    for i in range(length):
        for j in range(len(edge_numbering[i])-1):
            # Append reaction keys for the scrambling on the individual edges
            all_reactions.append('H'+edge_numbering[i][j]+'to'+edge_numbering[i][j+1])
            all_reactions.append('H'+edge_numbering[i][j+1]+'to'+edge_numbering[i][j])
            if deuterium:
                all_reactions.append('D'+edge_numbering[i][j]+'to'+edge_numbering[i][j+1])
                all_reactions.append('D'+edge_numbering[i][j+1]+'to'+edge_numbering[i][j])
        ...
        if deuterium:
            all_reactions.append('D'+edge_numbering[i][-1]+'to'+link_numbering[i][0])
            all_reactions.append('D'+link_numbering[i][0]+'to'+edge_numbering[i][-1])
        # take last element of link for moving anticlockwise (implemented in case of bay regions)
        k = len(link_numbering[i-1])
        all_reactions.append('H'+edge_numbering[i][0]+'to'+link_numbering[i-1][k-1])
        all_reactions.append('H'+link_numbering[i-1][k-1]+'to'+edge_numbering[i][0])
        if deuterium:
            all_reactions.append('D'+edge_numbering[i][0]+'to'+link_numbering[i-1][k-1])
            all_reactions.append('D'+link_numbering[i-1][k-1]+'to'+edge_numbering[i][0])
...
def Check_available_rates(rates, molecule, warn_setting):
    ...
    needed_rates = Determine_needed_rates(molecule.Deuterium, molecule.edge_numbers,  molecule.link_numbers)
  
    for key in needed_rates:
        if key not in rates.reactionrates.keys():
            if warn_setting in ('w', 'W'):
                logger.warning('No rates found for '+key+', continuing without rates for '+key+'.')
